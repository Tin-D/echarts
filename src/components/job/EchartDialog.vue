    ...
    export default {
        ...
        data () {
            return {
                ...
                dialogVisible: true,
                chartData: {
                    columns: ['createTime', '电压', '总电压', '平均电压'],
                    rows: [],
                },
                refrigeration: {
                    areaStyle: {
                    ...
                    tooltip: {
                        ...
                        formatter: (params) => {
                            ...
                            for (let i = 0; i < params.length; i++) {
                                res += '<div><span class="circle" style="background:' + params[i].color + ';"></span>' + '<span>' + params[i].seriesName + ':' + params[i].data + ' V</span></div>';
                            }
                            return res;
                        },
                    },
        ...
        methods: {
            ...
            async refrigerationLine () {
                const res = await this.$store.dispatch(actionTypes.voltageEcharts, {
                    id: this.id,
                    index: this.index,
                });
                this.chartData.rows = [];
                this.refrigeration.series[0].data = [];
                this.refrigeration.series[1].data = [];
                this.refrigeration.series[2].data = [];
                this.refrigeration.xAxis.data = [];
                res.forEach((i) => {
                    this.refrigeration.series[0].data.push(i['电压']);
                    this.refrigeration.series[1].data.push(i['总电压']);
                    this.refrigeration.series[2].data.push(i['平均电压']);
                    this.refrigeration.xAxis.data.push(i.createTime);
                    this.chartData.rows.push(i);
                });
                this.$nextTick(() => {
                    ...
                    myChart.on('dataZoom', (event) => {
                        if (event.batch) {
                            ...
                            this.refrigeration.dataZoom[0].start = this.dataZoom.start;
                            this.refrigeration.dataZoom[0].end = this.dataZoom.end;
                        } else {
                            // 滑块缩放
                            this.dataZoom = {
                                start: event.start,
                    ...
                    myChart.setOption(this.refrigeration);
